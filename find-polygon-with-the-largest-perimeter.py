...
class Solution:
    def largestPerimeter(self, nums: list[int]) -> int:
        ...
        sides_considered = 1
        starting_perimeter = nums[0]

        for i in range(1, len(nums)):
            starting_perimeter += nums[i]
            sides_considered += 1

            if sides_considered >= 3 and starting_perimeter - nums[i] > nums[i]:
                valid_perimeters.append(starting_perimeter)

        return max(valid_perimeters) if valid_perimeters else -1
